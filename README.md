...
這是一份**由人與 AI 協作撰寫的原創自學教材**，把 Vibe Coding 的學習整理成一條清楚的路徑（不是外部教學的彙編）。你不需要先成為程式高手，只要會用電腦、願意動手，就能循序漸進地學會「用 AI 把想法變成程式」。
...
本教材由作者與 AI 協作撰寫。範例程式碼的邏輯與預期輸出經人工逐項核對，格式則由 `scripts/check-units.mjs` 檢查。這是一個**新發布、尚未經大量外部使用者驗證**的專案；若發現錯誤，歡迎開 Issue 回報。
...
3. 可直接執行的範例放在 [`examples/`](./examples/) 目錄（目前有 `text-tool/`），其餘單元的範例內嵌於各單元正文。
